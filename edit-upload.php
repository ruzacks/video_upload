...
$stmt = $conn->prepare("SELECT id, nik, id_kecamatan, id_desa, video_name, extension FROM videos WHERE id=?");
...
if ($result->num_rows > 0) {
    ...
    // Assuming $data->nik contains the file name (e.g., '1234567890123456.mp4')
    $videoPath = 'videos/' . $data->video_name . '.' . $data->extension;
                     ...
                     <div class="iq-card-body">
                        <form id="dataForm">
                           <div class="form-group">
                              ...
                              <input type="text" class="form-control" id="nik" name="nik" maxlength="16" minlength="16" pattern="\d{16}" inputmode="numeric" value="<?= $data->nik ?>" required>
                           ...
                           <div class="form-group" id="kecamatan_input">
                                    ...
                                    <?php
                                       foreach ($kecamatans as $kecamatan){
                                          ?>
                                             <option value="<?php echo $kecamatan->id_kecamatan; ?>" <?= ($data->id_kecamatan == $kecamatan->id_kecamatan) ? 'selected' : ''; ?>> <?php echo $kecamatan->nama_kecamatan; ?></option>
                              ...
                              <input type="hidden" id="idVideo" name="idVideo" value="<?= $data->id ?>">                           
                     ...
                     <div class="iq-card-body">
                        <div class="row mt-4">
                           <div class="col-lg-12 col-md-6 text-center canvas-container">
                              <p class="iq-bg-secondary pt-5 pb-5 text-center rounded font-size-18" id="frame-template" onclick="document.getElementById('videoInput').click();" style="cursor:pointer" hidden>Take A Video</p>
                              <video id="videoPlayer" src="<?= $videoPath ?>" style="width: 100%; height: auto;" controls></video>
      ...
      <script>
        ...
        function getMessage(message) {
            ...
            $('#dataForm').on('submit', function(event) {
                ...
                $.ajax({
                    url: 'edit.php', // Endpoint to send the form data
                    type: 'POST',
                    data: formData,
